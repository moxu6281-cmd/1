...
const { createFallbackConfig } = require("./config/fallback");
...
function isLocalPreviewApi(url) {
...
App({
  globalData: {
    ...
    allowPreviewFallback: env.allowPreviewFallback !== false,
    customerId: "",
    customerToken: "",
    customer: null,
    customerReady: null,
  ...
  onLaunch() {
    ...
    if (ext && typeof ext.allowPreviewFallback === "boolean") {
      this.globalData.allowPreviewFallback = ext.allowPreviewFallback;
    }
    this.globalData.allowPreviewFallback = this.globalData.allowPreviewFallback
      && isLocalPreviewApi(this.globalData.apiBaseUrl);
  ...
  async ensureCustomerIdentity(force = false) {
    ...
    const task = (async () => {
      try {
        ...
        if (this.globalData.allowPreviewFallback && cached && cached.displayName && cached.numericId) {
          this.globalData.customer = cached;
          return cached;
        }
        throw error;
      } finally {
  ...
  async loadConfig(force = false) {
    ...
    try {
      ...
      this.globalData.previewFallback = false;
      this.globalData.configError = "";
    } catch (error) {
      if (!this.globalData.allowPreviewFallback) throw error;
      raw = createFallbackConfig();
      this.globalData.previewFallback = true;
  ...
  async loadOrders() {
    try {
      ...
      return await api.getOrders(this.globalData.merchantAccount);
    } catch (error) {
      if (this.globalData.previewFallback) return [];
      throw error;
    }
  }
});
