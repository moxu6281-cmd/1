...
Page({
  ...
  async loadProduct(force) {
    try {
      ...
      this.setData({
        loading: false, product, config, servers,
        serverIndex: servers.length ? Math.min(Math.max(0, Number(this.data.serverIndex) || 0), servers.length - 1) : 0, displayPrice, payPrice,
